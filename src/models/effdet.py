...
from src.models import bifpn as fpn_configs
from src.models import util_keras
...
class FNode(tf.keras.layers.Layer):
    ...
    def fuse_features(self, nodes):
        ...
        if self.weight_method == 'attn':
            ...
            edge_weights = [tf.cast(var, dtype=dtype) for var in self.vars]
            normalized_weights = tf.nn.softmax(
                tf.stack(edge_weights, -1), axis=-1)
            nodes = tf.stack(nodes, axis=-1)
            new_node = tf.reduce_sum(nodes * normalized_weights, -1)
    ...
    def build(self, feats_shape):
        for i, input_offset in enumerate(self.inputs_offsets):
            name = 'resample_{}_{}_{}'.format(
                i, input_offset, len(feats_shape))
...
class OpAfterCombine(tf.keras.layers.Layer):
    ...
    def call(self, new_node, training):
        if not self.conv_bn_act_pattern:
            new_node = util_keras.activation_fn(new_node, self.act_type)
...
class ResampleFeatureMap(tf.keras.layers.Layer):
    ...
    def _pool2d(self, inputs, height, width, target_height, target_width):
        ...
        if self.pooling_type == 'avg':
            return tf.keras.layers.AveragePooling2D(
                ...
                data_format=self.data_format)(inputs)
        raise ValueError(
            'Unsupported pooling type {}.'.format(self.pooling_type))
    ...
    def call(self, feat, training, all_feats):
        hwc_idx = (2, 3, 1) if self.data_format == 'channels_first' else (
            1, 2, 3)
        ...
        if height > target_height and width > target_width:
            ...
            feat = self._pool2d(feat, height, width,
...
class ClassNet(tf.keras.layers.Layer):
    ...
    def _conv_bn_act(self, image, i, level_id, training):
        ...
        @util_keras.recompute_grad(self.grad_checkpoint)
        def _call(image):
            original_image = image
            image = conv_op(image)
            image = bn(image, training=training)
            if self.act_type:
                image = util_keras.activation_fn(image, self.act_type)
            if i > 0 and self.survival_prob:
                image = util_keras.drop_connect(image, training, self.survival_prob)
...
class SegmentationHead(tf.keras.layers.Layer):
    ...
    def call(self, feats, training):
        ...
        for con2d_t, con2d_t_bn, skip in zip(self.con2d_ts, self.con2d_t_bns,
            ...
            x = util_keras.activation_fn(x, self.act_type)
...
class FPNCell(tf.keras.layers.Layer):
    ...
    def call(self, feats, training):
        @util_keras.recompute_grad(self.config.grad_checkpoint)
        def _call(feats):
...
class EfficientDetNet(tf.keras.Model):
    ...
    def __init__(self,
        ...
        if 'efficientnet' in backbone_name:
            override_params = {
                'batch_norm':
                    util_keras.batch_norm_class(is_training_bn, config.strategy),
                'relu_fn':
                    functools.partial(util_keras.activation_fn,
    ...
    def call(self, inputs, training):
        ...
        all_feats = self.backbone(
            inputs, training=training, features_only=True)
...
class EfficientDetModel(EfficientDetNet):
  ...
  def _preprocessing(self,
    ...
    image_size = util_keras.parse_image_size(image_size)
    if mode != 'infer':
