...
from src.models.backbones import efficientnet_model
...
def efficientnet(width_coefficient=None,
    ...
    global_params = efficientnet_model.GlobalParams(
        ...
        batch_norm=tf.keras.layers.BatchNormalization, #utils.TpuBatchNormalization,  # TPU-specific requirement.
        use_se=True,
...
def build_model(images,
    ...
    if override_params and override_params.get('drop_connect_rate', None):
        override_params['survival_prob'] = 1 - \
            override_params['drop_connect_rate']

    if not training or fine_tuning:
        if not override_params:
            override_params = {}
        override_params['batch_norm'] = tf.keras.layers.BatchNormalization #utils.BatchNormalization
        if fine_tuning:
