...
class SE(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        h_axis, w_axis = [
            2, 3] if self._data_format == 'channels_first' else [1, 2]
...
class MBConvBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training, survival_prob=None):
        ...
        Returns:
          A output tensor.
        """
        @util_keras.recompute_grad(self._global_params.grad_checkpoint)
        def _call(inputs):
            ...
            if self._block_args.id_skip:
                if all(
                    ...
                    if survival_prob:
                        x = util_keras.drop_connect(x, training, survival_prob)
                    x = tf.add(x, inputs)
