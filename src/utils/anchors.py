...
def anchor_targets_bbox(
    ...
    if annotations[:, :4].shape[0]:
        ...
        labels_batch[positive_indices, annotations[:, 4:][argmax_overlaps_inds[positive_indices]].astype(int)] = 1

        regression_batch[:, :4] = bbox_transform(
            anchors, annotations[:, :4][argmax_overlaps_inds, :])
...
def layer_shapes(image_shape, model):
    ...
    for layer in model.layers[1:]:
        ...
        for node in nodes:
            input_shapes = [shape[inbound_layer.name]
...
def generate_anchors(base_size=16, ratios=None, scales=None):
    ...
    anchors[:, 2:] = base_size * \
        np.tile(np.repeat(scales, len(ratios))[None], (2, 1)).T
